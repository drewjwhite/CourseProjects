...
where MovieID = @movieID and SessionDate >= @startDate and SessionDate < @endDate";
        ...
        public static List<Session> GetAllSessionsInTheFuture()
        {
            ...
            {
                conn.Open();
                string sql = @"select SessionID, MovieID, SessionDate, CinemaNumber from [Session] where SessionDate > @date";
                using (SqlCommand command = new SqlCommand(sql, conn))
                {
                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            session.MovieID = reader.GetInt32(1);
                            session.SessionDate = reader.GetDateTime(2);
                            session.CinemaNumber = reader.GetInt32(3);
                            sessions.Add(session);
                        }
                    }
                }
        ...
        public static Session GetSessionByID(int sessionID)
        {
            ...
            {
                conn.Open();
                string sql = @"select SessionID, MovieID, SessionDate, CinemaNumber from [Session] where SessionID = @session";
                using (SqlCommand command = new SqlCommand(sql, conn))
                {
...
values (@movieID, @title)";
        ...
        public static void AddSession(Session session)
        {
            ...
            {
                conn.Open();
                string sql = @"insert into [Session] (SessionID, MovieID, SessionDate, CinemaNumber) values (@sessionID, @movieID, @sessionDate, @cinemaNumber)";
                using (SqlCommand command = new SqlCommand(sql, conn))
                {
...
where MovieID=@movieID";
        ...
        public static void UpdateSession(Session session)
        {
            ...
            {
                conn.Open();
                string sql = @"update [Session] set MovieID=@movieID, SessionDate=@sessionDate, CinemaNumber=@cinemaNumber where SessionID=@sessionID";
                using (SqlCommand command = new SqlCommand(sql, conn))
                {
                    command.Parameters.AddWithValue("movieID", session.MovieID);
                    command.Parameters.AddWithValue("sessionDate", session.SessionDate);
