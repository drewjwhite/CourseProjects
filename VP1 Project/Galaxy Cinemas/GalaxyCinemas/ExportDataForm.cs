...
namespace GalaxyCinemas
{
    public partial class ExportDataForm : Form
    {
        ...
        /// <summary>
        /// Selects location and populates the save location text box 
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btnBrowse_Click(object sender, EventArgs e)
        {
            SaveFileDialog sfdb = new SaveFileDialog();
            sfdb.InitialDirectory = @"C:\";
            sfdb.Title = "Export .csv File";
            sfdb.CheckFileExists = true;
            sfdb.CheckPathExists = true;
            sfdb.DefaultExt = "csv";
            sfdb.Filter = "Csv files (*.csv)|*.csv|All files (*.*)|*.*";
            if (sfdb.ShowDialog() == DialogResult.OK)
            {
                txtFileBooking.Text = sfdb.FileName;
            }
            txtFileBooking.Focus(); // Set focus on this field. Moving focus will force validation of the value.
        }
